...
#if defined(_WIN32)
#include "win.hpp"
#define __ENUMIVO_OS__ sys::win

#elif defined(__linux__) || defined(__CYGWIN__)
#include "linux.hpp"
#define __ENUMIVO_OS__ sys::linux

#elif defined(__APPLE__)
#include "osx.hpp"
#define __ENUMIVO_OS__ sys::osx

#elif defined(__DragonFly__) || defined(__FreeBSD__) || \
      defined(__FreeBSD_kernel__) || defined(__NetBSD__)
#include "bsd.hpp"
#define __ENUMIVO_OS__ sys::bsd
...
struct whereami {
   static inline int getExecutablePath(char* out, int capacity, int* dirname_length) {
      return _getExecutablePath<__ENUMIVO_OS__>(out, capacity, dirname_length);
   }

   static inline int getModulePath(char* out, int capacity, int* dirname_length) {
      return _getModulePath<__ENUMIVO_OS__>(out, capacity, dirname_length);
   }
